...
def update_task(task_id : int, details : UpdateTask):
    if details.title is not None and not details.title.strip():
        raise HTTPException(status_code = 400, detail = "Title cannot be empty" )
    curr_task = None
    for task in tasks:
        if(task["id"] == task_id):
    ...
    if details.title is not None:
        curr_task["title"] = details.title
    if details.done is not None:
        curr_task["done"] = details.done
    return curr_task
